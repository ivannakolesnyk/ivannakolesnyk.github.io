...
public class UserController {
    ...
    public ResponseEntity<?> getProfile(
        ...
        User found = this.userRepository.findByEmail(username).orElse(null);

        if (found != null) {
            UserProfileDTO profile = new UserProfileDTO();
            profile.setName(found.getName());
            profile.setEmail(found.getEmail());
            profile.setPhone_number(found.getPhone_number());
            profile.setPostal_code(found.getPostal_code());
            profile.setAddress(found.getAddress());
            profile.setCity(found.getCity());
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }

        if (sessionUser.getEmail().equals(username) || sessionUser.isAdmin()) {
            return new ResponseEntity<>(found, HttpStatus.OK);
        } else if (sessionUser == null) {
            return new ResponseEntity<>("Profile data accessible only to authenticated users",
                    HttpStatus.UNAUTHORIZED);
        } else {
            return new ResponseEntity<>("Profile data for other users are accessible only by admins!",
